...
CREATE TABLE IF NOT EXISTS `mydb`.`LOG_PLN_CLI` (
  ...
  INDEX `fk_LOG_PLN_CLI_PLANNING1_idx` (`PLN_JOUR` ASC) VISIBLE,
  INDEX `fk_LOG_PLN_CLI_LOGEMENT1_idx` (`LOG_ID` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`EMAIL` (
  ...
  INDEX `fk_EMAIL_CLIENT_idx` (`CLI_ID` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`TELEPHONE` (
  ...
  INDEX `fk_TELEPHONE_CLIENT1_idx` (`CLI_ID` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`ADRESSE` (
  ...
  INDEX `fk_ADRESSE_CLIENT1_idx` (`CLI_ID` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`TRF_LOG` (
  ...
  INDEX `fk_TRF_LOG_LOGEMENT1_idx` (`LOG_ID` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`TARIF` (
  ...
  CONSTRAINT `fk_TARIF_TRF_LOG1`
    FOREIGN KEY (`TRF_DATE_DEBUT`)
    REFERENCES `mydb`.`TRF_LOG` (`TRF_DATE_DEBUT`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`FACTURE` (
  ...
  INDEX `fk_FACTURE_CLIENT1_idx` (`CLI_ID` ASC) VISIBLE,
  CONSTRAINT `fk_FACTURE_CLIENT1`
    FOREIGN KEY (`CLI_ID`)
    REFERENCES `mydb`.`CLIENT` (`CLI_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_FACTURE_LIGNE_FACTURE1`
    FOREIGN KEY (`FCT_ID`)
    REFERENCES `mydb`.`LIGNE_FACTURE` (`FCT_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
